...
public static class TgaWriter
{
    ...
    private static void WriteRle(FileStream stream, byte[] buffer, int pixelSize)
    {
        ...
        while (offset < buffer.Length)
        {
            ...
            stream.WriteByte((byte)(runLength - 1));
            stream.Write(buffer, rawStart, runLength * pixelSize);
            offset += runLength * pixelSize;
        }
    }

    private static bool PixelsEqual(byte[] buffer, int offsetA, int offsetB, int pixelSize)
    {
        for (var i = 0; i < pixelSize; i++)
        {
            if (buffer[offsetA + i] != buffer[offsetB + i])
                return false;
        }
